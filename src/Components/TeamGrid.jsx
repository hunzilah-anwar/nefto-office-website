...
const PremiumCard = ({ data }) => {
  ...
  return (
    ...
    >
      ...
      {/* GRADIENTS */}
      <div className="absolute inset-0 bg-linear-to-t from-(--color-indigo-bright) via-black/50 to-transparent opacity-60 translate-y-[40%] transition-transform duration-500 group-hover:translate-y-[20%] group-hover:opacity-90 mix-blend-overlay" />
      ...
      >
        ...
        <div className="mb-2">
          <p className="text-primary-soft font-bold text-xs tracking-[3px] uppercase">
            {data.category}
          </p>
        </div>
        ...
        <div className="grid grid-rows-[0fr] group-hover:grid-rows-[1fr] transition-[grid-template-rows] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">
          <div className="overflow-hidden">
            <p className="text-gray-300 text-sm leading-relaxed py-1 border-l-2 border-(--color-mint-soft) pl-4">
              {data.description}
            </p>
          </div>
        </div>
      ...
      <div className="relative max-w-350 mx-auto z-10">
        ...
        <div className="text-center mb-20">
          <h2 className="text-5xl md:text-6xl font-black tracking-tighter text-black mb-2">
            MEET THE{" "}
            <span className="text-transparent bg-clip-text bg-(image:--grad-hero)">
              TEAM
            </span>
          </h2>
          <div className="h-1 w-24 bg-(image:--grad-blue-gold-split) mx-auto" />
        </div>
